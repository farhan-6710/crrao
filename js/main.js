...
/* ── Lead-capture popup (exit intent + delayed) ──────────────────
   Shows once per session via sessionStorage flag. Triggered by:
     1. Mouse leaving viewport at the top (desktop only)
     2. 35-second time delay (mobile fallback)
     3. Manual call: window.openLeadPopup()                       */
(function initLeadPopup() {
  const SESSION_FLAG = 'crrao-lead-popup-shown';
  ...
  const popupHTML = `
    <div class="lead-popup" id="leadPopup" role="dialog" aria-modal="true" aria-labelledby="leadPopupTitle">
      <div class="lead-popup-inner">
        <button class="lead-popup-close" id="leadPopupClose" aria-label="Close">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
        <div class="lead-popup-header">
          <h3 id="leadPopupTitle">Wait — don't miss your seat</h3>
          <p>B.Tech 2026-27 admissions are filling fast. Get a free 15-min counsellor call before you decide.</p>
        </div>
        <div class="lead-popup-body">
          <form class="admission-form" data-validate id="leadPopupForm">
            <div class="form-group">
              <label class="form-label" for="lp-name">Name *</label>
              <input class="form-input" id="lp-name" name="name" type="text" placeholder="Student name" required />
            </div>
            <div class="form-row">
              <div class="form-group">
                <label class="form-label" for="lp-phone">Mobile *</label>
                <input class="form-input" id="lp-phone" name="phone" type="tel" placeholder="10-digit mobile" required />
              </div>
              <div class="form-group">
                <label class="form-label" for="lp-program">Program</label>
                <select class="form-select" id="lp-program" name="program">
                  <option value="">— Select —</option>
                  <option>B.Tech AI &amp; ML</option>
                  <option>B.Tech Data Science</option>
                  <option>B.Tech Cyber Security</option>
                  <option>B.Tech CSE</option>
                  <option>B.Tech Math &amp; Computing</option>
                </select>
              </div>
            </div>
            <input type="hidden" name="form_type" value="Exit-intent popup" />
            <button type="submit" class="btn btn-primary form-submit">
              Get a Free Counsellor Call
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" width="16" height="16"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </button>
            <p class="form-consent" style="margin-top:0.6rem;">No spam, just a one-time call. By submitting you agree to be contacted by CR Rao AIMSCS.</p>
          </form>
        </div>
      </div>
    </div>`;
  ...
  const close = document.getElementById('leadPopupClose');
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape' && popup.classList.contains('open')) dismiss(); });

  /* Re-wire validation for the popup form (forms.querySelectorAll
     already ran before this popup was injected). */
  if (form) {
    ...
    form.addEventListener('submit', e => {
      ...
      inputs.forEach(input => {
        const val = input.value.trim();
        let bad = (input.required && !val)
          || (input.type === 'tel' && val && !/^[6-9]\d{9}$/.test(val.replace(/\s/g, '')));
        if (bad) { input.classList.add('error'); valid = false; }
      ...
      setTimeout(() => {
        form.innerHTML = `
          <div style="text-align:center;padding:1rem 0.5rem;">
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" style="margin:0 auto 0.6rem;display:block;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            <h4 style="margin-bottom:0.3rem;">Thanks — we'll call you in 24 hours</h4>
            <p style="color:#4b5563;font-size:0.9rem;margin:0;">Our admissions counsellor will reach out shortly with program details and next steps.</p>
          </div>`;
  ...
  let armed = false;
  const armTimer = setTimeout(() => { armed = true; }, 12000); // grace period
  ...
  /* Trigger 2: time delay (mobile / fallback) */
  setTimeout(() => {
    if (sessionStorage.getItem(SESSION_FLAG) !== '1') open();
  }, 35000);

  /* Public hook for manual triggers */
